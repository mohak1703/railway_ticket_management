...
import { httpAddNewUser } from '../../hooks/requests';
...
const Registration = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (actualData.name && actualData.email && actualData.password && actualData.password_confirmation && actualData.tc !== null) {
        if(isValidEmailAddress(actualData.email)){
          if (actualData.password === actualData.password_confirmation) {
            ...
            if(success){
              ...
              setError({ status: true, msg: "Could Not Submit Form", type: 'error' })
            }        
          } else {
          ...
          setError({ status: true, msg: "Enter valid email address", type: 'error' })
        }
      }
    else {
